export const Footer = () => {
  return (
    <footer className="flex flex-col items-center justify-center text-center text-white bg-[rgba(0,0,0,0.8)] h-60 px-10 mt-50">
      <img className="w-50 " src="/images/logo/logo-transp.png" alt="" />
      <h2>K Móveis Planejados – Qualidade, design e funcionalidade para o seu ambiente.</h2>

      <button className="hover:cursor-pointer underline-expand mt-3 color">CONTATO</button>
    </footer>
  );
};
